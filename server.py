...
def _relative_upload_path(filename: str) -> str:
    """Return a forward-slash relative path stored in the database."""
...
def _delete_conversation_files(conv_id: str, doc_info: dict | None = None):
    """Remove uploaded files for a conversation from local storage."""
    if doc_info and doc_info.get("path"):
        file_path = _resolve_upload_path(doc_info["path"])
...
def api_delete_conversation(conv_id: str, user: dict = Depends(get_current_user)):
    ...
    if str(conv["user_id"]) != user["user_id"]:
        raise HTTPException(403, "Access denied")
    _delete_conversation_files(conv_id, conv.get("doc_info"))
...
async def api_upload_document(conv_id: str, file: UploadFile = File(...), user: dict = Depends(get_current_user)):
    ...
    stored_name = f"{conv_id}{suffix}"
    file_path = os.path.join(UPLOAD_DIR, stored_name)
    with open(file_path, "wb") as fout:
        fout.write(content)
    relative_path = _relative_upload_path(stored_name)
...
async def api_get_file(conv_id: str, user: dict = Depends(get_current_user)):
    ...
    doc_info = conv["doc_info"]
    if doc_info.get("path"):
        file_path = _resolve_upload_path(doc_info["path"])
    else:
        ext = doc_info["type"].lower()
